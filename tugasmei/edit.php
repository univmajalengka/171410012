<?php
include"koneksi.php";
$query="SELECT * FROM tabel_kelengkapan WHERE no='$_GET[id]'";
$galeri=mysqli_query($koneksi,$query) or die('error,query failed.'.mysqli_error());
$m=mysqli_fetch_array($galeri);
?>

<h3 align="center">Edit Kelengkapan</h3>
<form class="form-horizontal" method="post" action="halamankelengkapan_up" enctype="multipart/form-data">
	<input type="hidden" name="id" vaklue="<?php echo"$m[no]";?>"/>
<div class="form-group">
	<label class="col-sm-2 control-label">STTB : </label>
	<div class="col-sm-3">
	<input type="text" name="STTB" class="form-control" placeholder="masukan STTB" value="<?php echo"$m[STTB]";?>" requred>
	</div>
</div>
	<div class="form-group">
